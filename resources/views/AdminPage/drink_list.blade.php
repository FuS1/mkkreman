...
<head>
	...
	<script>
		$(function() {
			var table = initTabulator("#drink-table",{
				rowHeight:200,
				initialSort:[
						...
						formatter: function(cell, formatterParams) {
							return  '<div class="inline-flex">'+
										...
										'<button class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded-r" onclick="changeSeminarMediaSort('+cell.getRow().getData()['id']+',\'backward\')">'+
											'排序往後'+
										'</button>'+
										'<a href="./drink?drink_id='+cell.getRow().getData()['id']+'" class="bg-[#062851] hover:bg-[#03152b] text-gray-100 font-bold py-2 px-4 rounded">'+
											'編輯'+
										'</a>'+
				...
				ajaxURL: ENV['APP_API_URL'] + "admin/tabulator/drink",
		...
		var deleteSeminarMedia = function(drink_id){
			exec('drink','DELETE',{
				drink_id:drink_id
			},function(response){
				console.log(response);
		...
		var changeSeminarMediaSort = function(drink_id,action){
			exec('drink/sort','PUT',{
				drink_id:drink_id,
				action:action,
			},function(response){
...
<body class="flex bg-gray-50">
	...
	<div class="md:w-2/3 lg:w-3/4 xl:w-5/6 2xl:w-6/7">
		<div class="py-5 px-5 w-full">
			<div class="mb-2">
				<h1 class="text-1xl md:text-2xl font-bold text-gray-600 inline-block align-middle">清爽飲品商品設定</h1>
			</div>
			<hr class="my-6 h-px bg-gray-200 border-0 dark:bg-gray-700">
			<a href="./drink" class="px-6 py-3 text-white font-medium rounded-lg text-sm no-underline bg-[#062851] hover:bg-[#03152b] hover:text-blue-200">增加清爽飲品</a>
			<hr class="my-2 h-px bg-gray-200 border-0 dark:bg-gray-700">
			<div class="table-responsive" id="drink-table"></div>
		</div>
	</div>
</body>
</html>
