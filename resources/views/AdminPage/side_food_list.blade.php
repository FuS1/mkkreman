...
<head>
	...
	<script>
		$(function() {
			var table = initTabulator("#sidefood-table",{
				rowHeight:200,
				initialSort:[
						...
						formatter: function(cell, formatterParams) {
							return  '<div class="inline-flex">'+
										...
										'<button class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded-r" onclick="changeSeminarMediaSort('+cell.getRow().getData()['id']+',\'backward\')">'+
											'排序往後'+
										'</button>'+
										'<a href="./side_food?sidefood_id='+cell.getRow().getData()['id']+'" class="bg-[#062851] hover:bg-[#03152b] text-gray-100 font-bold py-2 px-4 rounded">'+
											'編輯'+
										'</a>'+
				...
				ajaxURL: ENV['APP_API_URL'] + "admin/tabulator/sidefood",
			...
			$("#btn-save-sidefood").on('click',function(){
				...
				let form = $("#add-sidefood-modal form");
		...
		var deleteSeminarMedia = function(sidefood_id){
			exec('sidefood','DELETE',{
		...
		var changeSeminarMediaSort = function(sidefood_id,action){
			exec('sidefood/sort','PUT',{
				sidefood_id:sidefood_id,
				action:action,
			},function(response){
				console.log(response);
...
<body class="flex bg-gray-50">
	...
	<div class="md:w-2/3 lg:w-3/4 xl:w-5/6 2xl:w-6/7">
		<div class="py-5 px-5 w-full">
			...
			<a href="./side_food" class="px-6 py-3 text-white font-medium rounded-lg text-sm no-underline bg-[#062851] hover:bg-[#03152b] hover:text-blue-200">增加美味小菜</a>
			<hr class="my-2 h-px bg-gray-200 border-0 dark:bg-gray-700">
			<div class="table-responsive" id="sidefood-table"></div>
		</div>
	</div>
</body>
</html>
