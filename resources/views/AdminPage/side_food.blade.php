...
<head>
	...
	<script>
		$(function() {
			...
			var sidefood_id = getUrlParam('sidefood_id');
			if(sidefood_id){
				exec('sidefood','GET',{
					sidefood_id : sidefood_id
				},function(response){
					sidefood_data = flattenObject(response);
					console.log(sidefood_data);
					for(var i in sidefood_data){
						$("[name="+replaceAll(i,".","\\.")+"]").each(function( index ) {
							if($(this).is('input') || $(this).is('select') ){
								$(this).val(sidefood_data[i]);
							}else if($(this).is('span')){
								$(this).text(sidefood_data[i]);
							}else if($(this).is('div')){
								$(this).html(sidefood_data[i]);
							}else if($(this).is('textarea')){
								$(this).val(sidefood_data[i].replaceAll('<br />',"\n"));
							}
						});
					}

					if(sidefood_data['file_url']){
						let img = $('#sidefood-form #sidefood_file').parent().find('img');
						img.attr('src',sidefood_data['file_url']);	
			...
			$(".btn-save-sidefood").on('click',function(){
				let form = $("#sidefood-form");
				if( form[0].reportValidity() ){
					let data = getFormData(form,{
						sidefood_id : sidefood_id,
						file	: form.find('#sidefood_file')[0].files[0],
					...
					console.log(data)
					exec('sidefood','POST',data,function(response){
						console.log(response);
						window.location.assign('./side_food_list');
					});
				}
...
<body class="flex bg-gray-50">
	...
	<div class="md:w-2/3 lg:w-3/4 xl:w-5/6 2xl:w-6/7">
		<div class="py-5 px-5 w-full">
			<div class="mb-2">
				<h1 class="text-1xl md:text-2xl font-bold text-gray-600 inline-block align-middle">美味小菜設定</h1>
			</div>
			<hr class="my-6 h-px bg-gray-200 border-0 dark:bg-gray-700">
		</div>
		<div class="container mx-auto px-8">
			<form id="sidefood-form">
				<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-1 mb-2">
					...
					<div class="justify-center">
						<label class="block border-l-8 border-l-[#062851] pl-2 py-1 mb-2 text-sm font-medium text-gray-900 dark:text-gray-300" for="sidefood_file">圖檔</label>
						...
						<input id="sidefood_file" class="block w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 cursor-pointer dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" aria-describedby="file_input_help" type="file">
		...
		<div class="container mx-auto mb-8 px-8 flex justify-center">
			<a href="./side_food_list" class="px-6 py-3 text-black font-medium rounded-lg text-sm no-underline bg-gray-200 hover:bg-gray-300">返回</a>	
			<button class="px-6 py-3 ml-6 text-white font-medium rounded-lg text-sm no-underline bg-cyan-800 hover:bg-cyan-900 btn-save-sidefood">儲存</button>	
		</div>
	</div>
</body>
</html>
