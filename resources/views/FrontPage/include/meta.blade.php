<meta charset="UTF-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>麵匡匡拉麵食堂</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11.6.16"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/moment.js/2.29.4/moment.min.js"></script>
<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@100;300;400;500;700;900&display=swap" rel="stylesheet">
<link rel="stylesheet" href="{{ asset('FrontPage/lib/css/bootstrap/bootstrap.css') }}">
<link rel="stylesheet" href="{{ asset('FrontPage/lib/css/swiper/swiper-bundle.min.css') }}">
<link rel="stylesheet" href="{{ asset('FrontPage/public/css/style.css') }}"> 
<meta property="og:type" content="type" />  
<meta property="og:title" content="{{ config('env.APP_CHINESE_NAME') }}" />
<meta property="og:description" content="{{ config('env.APP_CHINESE_DESCRIPTION') }}" />
<meta property="og:image" content="{{ mix('/FrontPage/public/img/favicon.png') }}" />
<meta property="og:url" content="{{ config('env.APP_URL') }}" />
<meta property="og:site_name" content="{{ config('env.APP_CHINESE_NAME') }}" />
<link rel="icon" href="{{ mix('/FrontPage/public/img/favicon.png') }}">
